...
| 版本 / Edition | 下载 / Download | 启动语言 / Default |
| --- | --- | --- |
| 中文版 · Windows 64 位 | [下载中文版 EXE](https://github.com/10walnut/serial-protocol-tester-app/releases/latest/download/SerialProtocolAssistant-ZH-CN.exe) | 中文界面 + 中文示例 |
| English · Windows x64 | [Download English EXE](https://github.com/10walnut/serial-protocol-tester-app/releases/latest/download/SerialProtocolAssistant-EN.exe) | English UI + English sample |

两个版本功能相同，均可在软件内切换语言，并分别记住语言选择。Both editions provide the same features and remember your language selection. User protocol annotations remain in the language of their JSON.

**从安装到联调的 13 步图解：[中文使用说明](docs/user-guide/README.md) · [English guide](docs/user-guide/README.en.md) · [离线教程与全部配图 ZIP](https://github.com/10walnut/serial-protocol-tester-app/releases/latest/download/SerialProtocolAssistant-User-Guide.zip)**
...
新版本提供 `SerialProtocolAssistant-ZH-CN.exe`（中文版）和 `SerialProtocolAssistant-EN.exe`（英文版）。GitHub 仓库地址和 `serial_protocol.v1` 协议格式不变，已有协议 JSON 可继续使用。
...
1. 从 [Releases](https://github.com/10walnut/serial-protocol-tester-app/releases/latest) 下载中文版 `SerialProtocolAssistant-ZH-CN.exe`。
...
.\build_serial_console.ps1 -Language en
```

打包脚本会生成多尺寸 Windows 图标、隔离 Qt DLL，并运行打包后的 `--self-test`。程序在正常启动时请求管理员权限。默认输出 `dist/SerialProtocolAssistant-ZH-CN.exe`，`-Language en` 输出 `dist/SerialProtocolAssistant-EN.exe`；中文版同时保留 `dist/SerialProtocolAssistant.exe` 兼容入口。
...
Use `SerialProtocolAssistant-EN.exe` for the English edition or `SerialProtocolAssistant-ZH-CN.exe` for Chinese. Repository URLs and the `serial_protocol.v1` format are unchanged, so existing protocol files continue to work.
...
1. Download `SerialProtocolAssistant-EN.exe` from the [latest release](https://github.com/10walnut/serial-protocol-tester-app/releases/latest) for an English interface and English example.
...
The build script defaults to the Chinese edition, `dist/SerialProtocolAssistant-ZH-CN.exe`. Pass `-Language en` for `dist/SerialProtocolAssistant-EN.exe`. It embeds the multi-resolution icon, isolates Qt dependencies, and runs a packaged self-test. The app requests administrator access during normal startup.
